...
(function () {
  ...
  if (contentEl) {
    contentEl.addEventListener('click', function () {
      if (window.innerWidth <= 960 && !sidebarCollapsed) {
  ...
  function loadArticle(name, preserveScroll) {
    ...
    // Clean up previous scroll spy and reading tracker
    ...
    return fetch('articles/' + name + '.md')
      ...
      .then(function (md) {
        ...
        updateTitle(name);
        var articleTitle = contentEl.querySelector('h1');
        showSidebarDetail(articleTitle ? articleTitle.textContent : articleSlugToTitle(name));
  ...
  function updateTitle(name) {
    ...
    if (topBarTitle) {
      topBarTitle.textContent = h1 ? h1.textContent : name;
    }
  }
  ...
  function onHash() {
    ...
    // Article-only hash — if already loaded, just show detail panel
    if (parsed.article === currentArticle) {
  ...
  function buildArticleList(slugs) {
    ...
    slugs.forEach(function (slug) {
      ...
      a.textContent = articleSlugToTitle(slug);
