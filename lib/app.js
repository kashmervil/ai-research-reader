...
(function () {
  ...
  function getStoredSlugs() {
    try { return JSON.parse(localStorage.getItem('articleSlugs') || '[]'); }
    catch (e) { return []; }
  }
  ...
  function getStoredArticle(slug) {
    return localStorage.getItem('article:' + slug);
  }

  function storeArticle(slug, content) {
    localStorage.setItem('article:' + slug, content);
    var slugs = getStoredSlugs();
  ...
  function removeStoredArticle(slug) {
    localStorage.removeItem('article:' + slug);
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      (function (file) {
        ...
        reader.onload = function (e) {
          storeArticle(filenameToSlug(file.name), e.target.result);
          done++;
          if (done === count) refreshArticleList();
        };
  ...
  function refreshArticleList() {
    buildArticleList(getStoredSlugs());
  ...
  function discoverArticles() {
    return Promise.resolve(getStoredSlugs());
  }
