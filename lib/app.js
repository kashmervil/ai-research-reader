...
(function () {
  ...
  var importBtn = document.getElementById('importToggle');
  var importInput = document.getElementById('importInput');
  var deleteBtn = document.getElementById('deleteArticle');
  ...
  // ─── Article storage (IndexedDB + localStorage fallback) ────────────────
  var DB_NAME = 'aiResearchReader';
  var DB_VERSION = 1;
      ...
      try {
        ...
        req.onupgradeneeded = function (e) {
          ...
          if (!idb.objectStoreNames.contains(STORE_ARTICLES)) {
            idb.createObjectStore(STORE_ARTICLES);
          }
  ...
  function migrateToIDB() {
    ...
    if (localStorage.getItem('idb_migrated') === 'true') return Promise.resolve();

    var slugs;
    try { slugs = JSON.parse(localStorage.getItem('articleSlugs') || '[]'); }
    catch (e) { slugs = []; }
    ...
    var tx = db.transaction([STORE_ARTICLES, STORE_META], 'readwrite');
    var articleStore = tx.objectStore(STORE_ARTICLES);
    var metaStore = tx.objectStore(STORE_META);

    slugs.forEach(function (slug) {
      var content = localStorage.getItem('article:' + slug);
      if (content) articleStore.put(content, slug);
    });
    metaStore.put(slugs, 'slugs');
  ...
  function getStoredSlugs() {
    if (db) {
      return idbGet(STORE_META, 'slugs').then(function (val) { return val || []; });
    }
    try { return Promise.resolve(JSON.parse(localStorage.getItem('articleSlugs') || '[]')); }
    catch (e) { return Promise.resolve([]); }
  }

  function setStoredSlugs(slugs) {
    if (db) {
      return idbPut(STORE_META, 'slugs', slugs);
    }
    localStorage.setItem('articleSlugs', JSON.stringify(slugs));
    return Promise.resolve();
  }

  function getStoredArticle(slug) {
    if (db) {
      return idbGet(STORE_ARTICLES, slug).then(function (val) { return val || null; });
    }
    return Promise.resolve(localStorage.getItem('article:' + slug));
  }

  function storeArticle(slug, content) {
    var store = db
      ? idbPut(STORE_ARTICLES, slug, content)
      : (localStorage.setItem('article:' + slug, content), Promise.resolve());
    return store.then(function () {
      ...
      if (slugs.indexOf(slug) === -1) {
        ...
        return setStoredSlugs(slugs);
      }
    });
  }

  function removeStoredArticle(slug) {
    localStorage.removeItem('readProgress:' + slug);
    var remove = db
      ? idbDelete(STORE_ARTICLES, slug)
      : (localStorage.removeItem('article:' + slug), Promise.resolve());
  ...
  function readFileAsText(file) {
    return new Promise(function (resolve) {
      ...
      reader.onload = function (e) {
        resolve({ slug: filenameToSlug(file.name), content: e.target.result });
      };
  ...
  function importFiles(files) {
    ...
    for (var i = 0; i < files.length; i++) {
      if (!files[i].name.match(/\.md$/i)) continue;
      pending.push(readFileAsText(files[i]));
    }
    ...
    Promise.all(pending).then(function (results) {
      ...
      results.forEach(function (r) {
        chain = chain.then(function () {
          return storeArticle(r.slug, r.content);
        });
      });
      return chain;
    }).then(function () {
  ...
  function deleteCurrentArticle() {
    ...
    if (!slug) return;
    removeStoredArticle(slug).then(function () {
  ...
  function refreshArticleList() {
    return getStoredSlugs().then(function (slugs) {
      buildArticleList(slugs);
      if (currentArticle) highlightActiveLink(currentArticle);
    });
  }
  ...
  function previewArticleTOC(slug) {
    ...
    return getStoredArticle(slug).then(function (md) {
      if (!md) { if (tocEl) tocEl.innerHTML = ''; return; }
  ...
  function loadArticle(name, preserveScroll) {
    if (!name) { return showWelcome(); }
    if (h1Observer) { h1Observer.disconnect(); h1Observer = null; }
    runCleanup();
    clearSearch();

    return getStoredArticle(name).then(function (md) {
      if (!md) {
        contentEl.innerHTML = '<div class="welcome"><h1>Not found</h1><p>Article "' + escapeHtml(name) + '" is not in storage.</p></div>';
        return;
      }
  ...
  function showWelcome() {
    if (h1Observer) { h1Observer.disconnect(); h1Observer = null; }
    return getStoredSlugs().then(function (slugs) {
      var msg = slugs.length > 0
        ? '<p>Select an article from the sidebar.</p>'
        : '<p>No articles yet. Use the <strong>+</strong> button in the sidebar to import <code>.md</code> files.</p>';
      contentEl.innerHTML = '<div class="welcome"><h1>AI Research Reader</h1>' + msg + '</div>';
  ...
  function onHash() {
    ...
    if (parsed.article && parsed.heading) {
      ...
      return loadArticle(parsed.article).then(function () {
    ...
    return loadArticle(parsed.article);
  }
  ...
  if (importBtn && importInput) {
    ...
    importInput.addEventListener('change', function () {
      if (importInput.files.length > 0) importFiles(importInput.files);
      importInput.value = '';
    });
  }
  ...
  if (deleteBtn) {
    deleteBtn.addEventListener('click', function () {
      var slug = previewSlug || currentArticle;
      if (!slug) return;
      if (confirm('Delete "' + articleSlugToTitle(slug) + '"?')) {
