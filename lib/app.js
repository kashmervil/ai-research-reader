...
(function () {
  ...
  function showSidebarDetail(title) {
    ...
    // Re-run search in new view context if search is active
    if (searchWrap && !searchWrap.hidden && searchInput && searchInput.value.trim()) {
      // Clear list filtering, apply detail search
  ...
  function showSidebarList() {
    ...
    // Re-run search in new view context if search is active
    if (searchWrap && !searchWrap.hidden && searchInput && searchInput.value.trim()) {
      // Clear TOC search results, apply list filtering
      if (tocEl) {
  ...
  // Detail title click — load article from beginning
  ...
  // ─── Preview TOC (show article's TOC without loading content) ──
  ...
  function previewArticleTOC(slug) {
    previewSlug = slug;
    var title = articleSlugToTitle(slug);
    showSidebarDetail(title);
    ...
    return fetch('articles/' + slug + '.md')
      ...
      .then(function (md) {
        ...
        // Build tree (same logic as buildTOC)
        var root = { children: [], level: 1 };
        var stack = [root];
        headings.forEach(function (h, i) {
          var id = 'heading-' + (i + 1);
          var node = { id: id, text: h.text, level: h.level, children: [] };
  ...
  function buildArticleList(slugs) {
    ...
    slugs.forEach(function (slug) {
      ...
      // Progress bar inside <a> so it's part of hover/selection
      var prog = getReadProgress(slug);
      ...
      a.addEventListener('click', function (e) {
        ...
        if (slug === currentArticle) {
          // Already loaded — show detail panel
          previewSlug = null;
          showSidebarDetail(articleSlugToTitle(slug));
          return;
        }
        if (currentArticle) {
          // Active content exists — preview TOC only, don't load content
          previewArticleTOC(slug);
        } else {
          // No active content — load immediately
          if (location.hash === '#' + slug) {
            onHash();
          } else {
            location.hash = slug;
  ...
  function clearSearch() {
    ...
    // Remove search result items and unhide TOC nodes
    if (tocEl) {
      tocEl.querySelectorAll('.toc-search-result').forEach(function (el) { el.remove(); });
      tocEl.querySelectorAll('.toc-search-hidden').forEach(function (el) { el.classList.remove('toc-search-hidden'); });
  ...
  function searchArticle(query) {
    ...
    var lowerQ = query.toLowerCase();
    var headings = contentEl.querySelectorAll('h2, h3, h4');
    var matchingIds = {}; // track which heading IDs have matches
    ...
    headings.forEach(function (h) {
      ...
      // Highlight matching TOC heading text
    ...
    var topLevelLis = tocEl.querySelectorAll(':scope > li');
    topLevelLis.forEach(function (li) { markVisible(li); });
  }
