...
(function () {
  ...
  function updateArticleProgressBar(slug) {
    ...
    bar.querySelector('.article-progress-fill').style.width = Math.min(prog.percent, 100) + '%';
  }
  ...
  function buildArticleList(slugs) {
    ...
    slugs.forEach(function (slug) {
      ...
      if (prog && prog.percent > 0) {
        ...
        bar.className = 'article-progress';
        bar.innerHTML = '<div class="article-progress-fill" style="width:' + Math.min(prog.percent, 100) + '%"></div>';
        li.appendChild(bar);
      }
